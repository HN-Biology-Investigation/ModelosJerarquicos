...
Leer [Rota et al. 2016](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12587) para mayor detalles de modelos de ocupación con multiples especies.
...
aictab(cand.set =  list(fm1, fm2, fm3, fm4, fm5),
       modnames = c("fm1", "fm2", "fm3", "fm4", "fm5"))
```
...
coyote <- predict(fm5,type = 'state',species = 'coyote',
    newdata = Dist_df, nsims = 1000 )

redfox <- predict(fm5,type = 'state',species = 'redfox',
    newdata = Dist_df, nsims = 1000)

bobcat <- predict(fm5,type = 'state',species = 'bobcat',
   newdata = Dist_df, nsims = 1000)
```
